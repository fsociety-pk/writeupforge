...
class WriteupForgeGUI(ctk.CTk):
    def __init__(self):
        ...
        self.settings_btn = ctk.CTkButton(self.sidebar_frame, text="⚙️ Settings", command=self.show_settings)
    ...
    def show_writeup_form(self):
        ...
        # Title
        ctk.CTkLabel(form_frame, text="📝 Writeup Title", font=ctk.CTkFont(size=12, weight="bold")).grid(row=0, column=0, sticky="w", pady=(15, 5))
        self.title_entry = ctk.CTkEntry(form_frame, placeholder_text="e.g., SQL Injection Challenge", height=40)
        self.title_entry.grid(row=1, column=0, sticky="ew", pady=5, padx=5)
        ctk.CTkLabel(form_frame, text="3-100 characters", text_color="gray", font=ctk.CTkFont(size=9)).grid(row=2, column=0, sticky="w", padx=5)

        # Author
        ctk.CTkLabel(form_frame, text="👤 Your Name", font=ctk.CTkFont(size=12, weight="bold")).grid(row=3, column=0, sticky="w", pady=(15, 5))
        self.author_entry = ctk.CTkEntry(form_frame, placeholder_text="Your full name", height=40)
        self.author_entry.grid(row=4, column=0, sticky="ew", pady=5, padx=5)
        ctk.CTkLabel(form_frame, text="This will appear in the report header", text_color="gray", font=ctk.CTkFont(size=9)).grid(row=5, column=0, sticky="w", padx=5)

        # Platform
        ctk.CTkLabel(form_frame, text="🎯 Platform / CTF", font=ctk.CTkFont(size=12, weight="bold")).grid(row=6, column=0, sticky="w", pady=(15, 5))
        self.platform_choice = ctk.CTkOptionMenu(
            form_frame,
            values=InputHandler.PLATFORMS,
            height=40
        )
        self.platform_choice.set("HackTheBox")
        self.platform_choice.grid(row=7, column=0, sticky="ew", pady=5, padx=5)
        ctk.CTkLabel(form_frame, text="Select from popular platforms or enter custom", text_color="gray", font=ctk.CTkFont(size=9)).grid(row=8, column=0, sticky="w", padx=5)

        # Difficulty
        ctk.CTkLabel(form_frame, text="⭐ Difficulty Level", font=ctk.CTkFont(size=12, weight="bold")).grid(row=9, column=0, sticky="w", pady=(15, 5))
        self.difficulty_choice = ctk.CTkOptionMenu(
            form_frame,
            values=InputHandler.DIFFICULTIES,
            height=40
        )
        self.difficulty_choice.set("Intermediate")
        self.difficulty_choice.grid(row=10, column=0, sticky="ew", pady=5, padx=5)
        ctk.CTkLabel(form_frame, text="Rate the challenge's difficulty level", text_color="gray", font=ctk.CTkFont(size=9)).grid(row=11, column=0, sticky="w", padx=5)

        # Notes
        ctk.CTkLabel(form_frame, text="📋 Lab Notes", font=ctk.CTkFont(size=12, weight="bold")).grid(row=12, column=0, sticky="w", pady=(15, 5))
        self.notes_text = ctk.CTkTextbox(form_frame, height=250)
        self.notes_text.grid(row=13, column=0, sticky="ew", pady=5, padx=5)
        self.notes_text.insert("1.0", "Paste your raw lab notes here...\n- Step 1: ...\n- Step 2: ...\n- Step 3: ...")
        ctk.CTkLabel(form_frame, text="Include reconnaissance, enumeration, exploitation steps, flags, and lessons learned", text_color="gray", font=ctk.CTkFont(size=9)).grid(row=14, column=0, sticky="w", padx=5)
    ...
    def show_settings(self):
        ...
        settings_frame = ctk.CTkScrollableFrame(self.main_container, label_text="⚙️ API Configuration")
        settings_frame.grid(row=0, column=0, sticky="nsew")
        settings_frame.grid_columnconfigure(0, weight=1)
        
        # Info box
        info_text = "Configure your Groq API key to enable AI-powered writeup generation.\n\nGet a free API key at: https://console.groq.com/keys"
        ctk.CTkLabel(
            settings_frame,
            text=info_text,
            text_color="cyan",
    ...
    def save_settings(self):
        ...
        if len(groq_key) < 10:
            messagebox.showerror("Error", "API key seems too short. Please verify it's correct.")
            return
    ...
    def test_api_connection(self):
        ...
        try:
            test_spinner = messagebox.showwarning("Testing", "Testing connection...", icon="info")
    ...
    def on_generate_click(self):
        ...
        # Validation
        if not title:
            messagebox.showerror("Error", "⚠️ Please enter a writeup title!")
            return
        
        if len(title) < 3:
            messagebox.showerror("Error", "⚠️ Title must be at least 3 characters!")
            return

        if not author:
            messagebox.showerror("Error", "⚠️ Please enter your name!")
            return
        
        if len(author) < 2:
            messagebox.showerror("Error", "⚠️ Name must be at least 2 characters!")
            return

        if not raw_notes or raw_notes == "Paste your raw lab notes here...\n- Step 1: ...\n- Step 2: ...\n- Step 3: ...":
            messagebox.showerror("Error", "⚠️ Please enter your lab notes!")
            return
    ...
    def process_writeup(self, title, author, platform, difficulty, raw_notes):
        try:
            ...
            self.after(100, lambda: self.gen_btn.configure(text="⚙️ AI Generation..."))
            ...
            md_filename = f"output/{title.replace(' ', '_').replace('/', '-')}_Writeup.md"
            with open(md_filename, "w", encoding="utf-8") as f:
                f.write(formatted_content)
            
            pdf_filename = f"output/{title.replace(' ', '_').replace('/', '-')}_Writeup.pdf"
    ...
    def on_processing_complete(self, md_path, pdf_path):
        ...
        message = f"✓ Success! Your professional writeup has been generated!\n\n" \
    ...
    def on_processing_error(self, error):
        self.progress_bar.grid_forget()
        self.gen_btn.configure(state="normal", text="✨ Generate Professional Report")
        messagebox.showerror("❌ Error", f"Failed to generate writeup:\n\n{error}\n\nMake sure your API key is configured in Settings.")
    ...
    def update_check_complete(self, has_update, latest_version):
        ...
        if latest_version is None:
            messagebox.showwarning("⚠️ Update Check", "Could not check for updates.\n\nPlease check your internet connection.")
            return
        
        if has_update:
            self.show_update_notification(latest_version)
        else:
            messagebox.showinfo("✓ Up to Date", f"You're running the latest version (v{CURRENT_VERSION})!")
